...
def managed_block_is_empty(text: str) -> bool:
    ...
    body = body.replace("## Managed AI Agent Behavior Options", "")
    body = body.replace(
    ...
    body = body.replace(
        "This section is generated by `AI + FluffMods`. Toggle options with `fluffmods` instead of editing this block by hand.",
        "",
    )
...
def render_block(enabled: set[str], options: tuple[Option, ...] = BUILTIN_OPTIONS) -> str:
    ...
    lines = [
        ...
        "",
        "## Managed AI Agent Behavior Options",
        "",
        "This section is generated by `fluffmods`. Toggle options with `fluffmods` instead of editing this block by hand.",
        "",
    ]
...
def build_agent_analysis_prompt(enabled: set[str], options: tuple[Option, ...]) -> str:
    return f"""Review these fluffmods configuration stanzas before they are trusted in an agent guidance file.
...
def run_agent_analysis(
    ...
    if result.returncode == 0 and output:
        return output
    detail = agent_error_summary(command, error or output, result.returncode)
...
def completed_agent_analysis(command: list[str], stdout: str | None, stderr: str | None, returncode: int | None) -> str:
    ...
    if returncode == 0 and output:
        return output
    detail = agent_error_summary(command, error or output, returncode)
    raise RuntimeError(detail)


def agent_error_summary(command: list[str], detail: str, returncode: int | None) -> str:
    if not detail.strip():
        return f"{command[0]} exited with status {returncode}"

    lines = [line.strip() for line in detail.splitlines() if line.strip()]
    priority_prefixes = ("ERROR:", "Error:", "error:", "Not logged in", "Authentication")
    priority = [line for line in lines if line.startswith(priority_prefixes)]
    selected = priority[-1] if priority else lines[-1]
    return compact_text(selected, 100)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "fluffmods: multi-agent guidance manager with curated feeds "
